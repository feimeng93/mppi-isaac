...
def sphere_occupancy(ko_joint_covariance: torch.Tensor,
                    rotatotopes: Union[Dict[str, Union[matPolyZonotope, batchMatPolyZonotope]],
    ...
    for name, val in link_fk_dict.items():
        ...
        joint_distribution = joint_uncertainty.clone()
        joint_distribution.Z *= 0
        joint_distribution.Z[:,1:,:] += ko_joint_covariance # shape torch.size([100,6,3]) 
        ko_joint_uncertainty = zp.batchZonotope(joint_distribution)
        uncertainty_rad = torch.linalg.vector_norm((joint_uncertainty+ko_joint_uncertainty).to_interval().rad(), dim=-1)
